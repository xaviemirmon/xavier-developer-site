          ...
          return (
            <a href={`https://medium.com/xaviemirmon/${data.uniqueSlug}`} target="_blank" rel="noreferrer" key={data.title} sx={{
              ...
              <Card className="card-with-overlay" key={data.title}>
                <GatsbyImage image={data.localImage.childImageSharp.gatsbyImageData} alt={`${data.title} promo image`} />
                ...
                {data.virtuals?.tags.map((tag) => {return ( <Badge mr={2} mb={2} key={`${data.title}-tag-${tag.name}`}>{tag.name}</Badge>)})}
              </Card>
              <Box id="overlay">
